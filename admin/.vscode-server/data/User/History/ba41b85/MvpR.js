...
app.get("/root/student/edit", (req, res) => {
    ...
    edit = Object.fromEntries(edit.split(',').map(entry => entry.trim().split(':').map(item => item.trim().replace(/^'|'$/g, ''))));
    ...
    jwt.verify(token, "robixe", (err, decoded) => {
        ...
        pool.query(`SELECT * FROM root WHERE id = ? AND user = ? AND pass = ?`, [id, user, pass], (err, result) => {
            ...
            // Remove id from the properties to update
            const { id, ...updateData } = edit;
            ...
            const values = [...Object.values(updateData), edit.id];
            ...
            pool.query(query, values, (err, result) => {
                ...
                return res.json({ code: 1, message: "Data updated successfully" });
            });
...
app.get("/root/student/delete", (req, res) => {
    
    const token = req.headers.authorization;
    let dele = req.headers.delete;
    dele = Object.fromEntries(dele.split(',').map(entry => entry.trim().split(':').map(item => item.trim().replace(/^'|'$/g, ''))));
    ...
    jwt.verify(token, "robixe", (err, decoded) => {
        ...
        pool.query(`SELECT * FROM root WHERE id = ? AND user = ? AND pass = ?`, [id, user, pass], (err, result) => {
            ...
            pool.query(query, [deleteData.id], (err, result) => {
                if (err) {
                    console.error("Database error:", err);
                    return res.json({ code: 0, message: "Database error", err });
                }

                return res.json({ code: 1, message: "Data deleted successfully" });
            });
